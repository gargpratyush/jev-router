...
import { routingBackend } from "./router.mjs";
...
export async function runCodex() {
  ...
  if (routingBackend() !== null) {
    const proxy = await startCodexProxy({ statusId });
    close = proxy.close;
    args = codexArgs(`http://127.0.0.1:${proxy.port}`, args);
  } else {
    process.stderr.write(
      "[jev] no routing key found - starting Codex without routing\n" +
        `[jev] add JEV_API_KEY=... to ${join(homedir(), ".jev-router.env")} (or OPENROUTER_API_KEY=...) and restart jev-codex\n`,
    );
  }
